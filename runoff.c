...
bool vote(int voter, int rank, string name)
{
    // Fill in the preference of each candidate
    for (int j = 0 ; j < candidate_count ; j++)
    {
...
void tabulate(void)
{
    ...
    for (int i = 0 ; i < voter_count ; i++)
    {
        for (int j = 0; j < candidate_count; j++)
        {
            ...
            if (candidates[test].eliminated == false)
            {
...
bool print_winner(void)
{
    // A candidate with more than halph of the votes is the winner
    int vote = round(voter_count / 2);
    for (int i = 0 ; i < candidate_count ; i++)
    {
...
bool is_tie(int min)
{
    // tells if every one has the same number of votes or not
    for (int i = 0 ; i < candidate_count ; i++)
    {
        if (!candidates[i].eliminated && candidates[i].votes != min)
        {
